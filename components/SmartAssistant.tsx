...
import { Sparkles, Send, CheckCircle, AlertCircle, WifiOff } from 'lucide-react';
      ...
      <div className="bg-gradient-to-r from-violet-600 to-indigo-600 rounded-2xl p-6 text-white shadow-lg">
        ...
        <div className="relative">
          <textarea
            ...
            disabled={isOffline}
            className="w-full bg-white/10 border border-white/20 rounded-xl p-3 text-white placeholder-indigo-200 focus:outline-none focus:ring-2 focus:ring-white/50 text-sm min-h-[100px] disabled:opacity-50 disabled:cursor-not-allowed"
            placeholder={isOffline ? t.aiOffline : t.aiPlaceholder}
          />
          <button 
            onClick={handleAnalyze}
            disabled={isLoading || !input.trim() || isOffline}
            className="absolute bottom-3 right-3 p-2 bg-white text-indigo-600 rounded-lg hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {isLoading ? (
        ...
        <div className="space-y-2">
          {[t.tip1, t.tip2, t.tip3].map((tip, i) => (
            <button 
              ...
              disabled={isOffline}
              className="block w-full text-left text-sm bg-white p-3 rounded-xl border border-slate-100 text-slate-600 hover:border-indigo-200 hover:text-indigo-600 transition-colors disabled:opacity-50 disabled:hover:border-slate-100 disabled:hover:text-slate-600"
            >
              "{tip}"
            </button>
          ))}
